...
@section('create_button')
    <div class="d-flex align-items-center justify-content-between filterPanelbtn my-2 flex-wrap">
        @permission('task-status.edit')
            <div></div>
            <a href="{{ route('task-status.edit') }}" class="btn-primary f-500 f-14">
        ...
        @forelse($statuses as $key => $status)
            <div class="card card-row card-secondary no-border">
                <div class="card-header" style="border-bottom: 2px solid {{ $status->color }};">
                    <h3 class="card-title">{{ strtoupper($status->name) }}</h3>
                </div>
                <div class="card-body-main drag-area" data-cardparent="{{ $status->id }}">
                    @if (isset($tasks[$status->id]))
                        @foreach ($tasks[$status->id] as $task)
                            <div class="card card-light card-outline mb-2 draggable-card portlet" data-cardchild="{{ $task['id'] }}" data-otitle="{{ $task['client'] }}">
                                <div class="card-body bg-white border-0 p-2 d-flex justify-content-between portlet-header">
                                    <div>
                                        <p class="color-blue">
                                            @if($task['status'] == "missed")
                                                <svg width="24" height="24" viewBox="0 0 24 24" fill="" xmlns="http://www.w3.org/2000/svg" class="text-danger"><path d="M5.707 4.293a1 1 0 0 0-1.414 1.414L11.586 13H7a1 1 0 1 0 0 2h5a3 3 0 0 0 3-3V7a1 1 0 1 0-2 0v4.586L5.707 4.293zM15.293 15.293a1 1 0 0 1 1.414 0l.793.793.793-.793a1 1 0 0 1 1.414 1.414l-.793.793.793.793a1 1 0 0 1-1.414 1.414l-.793-.793-.793.793a1 1 0 0 1-1.414-1.414l.793-.793-.793-.793a1 1 0 0 1 0-1.414z" fill="currentColor"></path></svg>
                                            @elseif($task['status'] == "success")
                                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="text-success"><path d="M4.293 4.293a1 1 0 0 1 1.414 0L13 11.586V7a1 1 0 1 1 2 0v5a3 3 0 0 1-3 3H7a1 1 0 1 1 0-2h4.586L4.293 5.707a1 1 0 0 1 0-1.414zM18 20a2 2 0 1 0 0-4 2 2 0 0 0 0 4z" fill="currentColor"></path></svg>
                                            @endif
                                            {{ \App\Helpers\Helper::getNumberFormated($task['client']) }}
                                        </p>
                                        <p class="no-m font-13">{{ ucfirst($task['status']) }}</p>
                                        <p class="no-m font-13">{{ ucfirst($task['user_name']) }}</p>
                                    </div>
                                    <div class="d-flex align-items-end flex-column" style="text-align: right;">
                                        <div class="card-date f-12 c-7b">
                                            <p class="no-m">{{ \Carbon\Carbon::parse($task['start'])->diffForHumans() }}</p>
                                            <p class="no-m">{{ \Carbon\Carbon::parse($task['start'])->setTimezone(config("services.timezones.europe_chisinau"))->format("d-m-Y H:i") }}</p>
                                            <p class="no-m">
                                                {{ \App\Helpers\Helper::getSecondsToMinutes($task['wait']) }} |  {{ \App\Helpers\Helper::getSecondsToMinutes($task['duration']) }}
                                            </p>
                                        </div>
                                    </div>
                                </div>
...
@section('script')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            let channel = pusher.subscribe('call-card-trigger');
            channel.bind('call-history-status-change', function(data) {
                ...
                if (shouldMove && 'element' in data) {
                    let toBeMovedAt = $(`[data-cardparent="${data.call_status}"]`);
                    $(toBeMovedAt).append(data.element);
                } else if (shouldMove && $(`[data-cardchild="${data.call_id}"]`).length > 0 && $(
                    `[data-cardparent="${data.call_status}"]`).length > 0) {
                    let toBeMoved = $(`[data-cardchild="${data.call_id}"]`).get(0);
                    let toBeMovedAt = $(`[data-cardparent="${data.call_status}"]`);

                    $(toBeMoved).find('.trigger-btn').attr('data-soid', data.call_status);

                    $(`[data-cardchild="${data.call_id}"]`).remove();
                    $(toBeMovedAt).append(toBeMoved)

                } else if ('call_old_status' in data && data.call_status == '1' && data.call_old_status == '2') {
                    if ($(`[data-cardchild="${data.call_id}"]`).length > 0) {
                        $(`[data-cardchild="${data.call_id}"]`).remove();
                    }
                    return false;
                } else if ('removing' in data && data.removing && data.call_status == '1') {
                    if ($(`[data-cardchild="${data.call_id}"]`).length > 0) {
                        $(`[data-cardchild="${data.call_id}"]`).remove();
                    }
                    return false;
            ...
            channel.bind('add-task-to-call-history', function(data) {
                toastr["info"](data.client, "Task added successfully")
            });
            ...
            $(document).on('click', '.draggable-card', function(event) {
                ...
                if (!($(event.target).hasClass('trigger-btn') || $(event.target).hasClass('fa-plus'))) {
                    if (thisOrderId != '' && thisOrderId != null) {
                        $.ajax({
                            url: "{{ route('task-status.call-history-detail') }}",
                            type: "POST",
                            data: {
                                id: thisOrderId
                            },
            ...
            $(".drag-area").sortable({
                ...
                receive: function(event, ui) {
                    ...
                    $.ajax({
                        url: "{{ route('task-status.sequence') }}",
                        type: 'POST',
                        data: {
                            'status': area,
                            'order': box,
                            ...
                            if ('responseJSON' in response && response.responseJSON) {
                                ...
                                if ('status' in response.responseJSON) {
                                    if (!response.responseJSON.status) {
                                        ...
                                        Swal.fire('', response.responseJSON.message, 'success');
                                    }
                                }
                            }
                        }
