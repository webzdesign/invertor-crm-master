    ...
    public function index() {
        ...
        foreach ($statuses as $status) {
            ...
            if ($tempOrder->exists()) {
                ...
                ->selectRaw("sales_orders.id, sales_orders.order_no, sales_orders.date, SUM(sales_order_items.amount) as amount, sales_orders.status as status, sales_orders.sold_amount as soldamount, sales_orders.driver_amount as driveramount")
                ...
                try {
                    if (SalesOrder::where('id', $request->order)->update(['status' => $request->status])) {
                        ...
                        if ($cwStatus == $request->status) {
                            ...
                            //Commission and driver fees
                        } else if ($request->status == $incomingOrderStatus) {
    ...
    public function rejectOrder(Request $request) {
        if (!empty($request->id)) {
            ...
            if ($order->exists()) {
                ...
                if (SalesOrder::where('id', $request->id)->where('responsible_user', auth()->user()->id)->exists()) {
                    SalesOrder::where('id', $request->id)->where('responsible_user', auth()->user()->id)->update([
